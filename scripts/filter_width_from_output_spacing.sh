...
function filter_width_from_output_spacing () {	# Accepts one argument only
	...
	K2D=$(gmt math -Q 2 ${SRC_RADIUS} MUL PI MUL 360 DIV =)		# Converts degrees on this planet to km
	FW_RAW=$(gmt math -Q ${1} ${K2D} MUL 2 MUL 2 SQRT MUL =)	# Filter width F_w given by our equation
